...
- `tiktok_fast_path.py` — чистый разбор ответа резолвера TikTok: прямая H.264-ссылка вместо yt-dlp, признак «`music` — это звук видео, а не библиотечный трек», проверка лимита передачи по URL. Включается `TIKTOK_FAST_PATH`, при отказе — откат на yt-dlp

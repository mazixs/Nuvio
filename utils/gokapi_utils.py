...
def upload_to_gokapi(file_path: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        return (
            False,
            "Сервер загрузки больших файлов не настроен. Обратитесь к администратору.",
        )
    try:
        ...
        with open(file_path, "rb") as f:
            ...
            logger.info(
                f"Отправка файла на Gokapi: {url}, имя: {file_path.name}, размер: {file_path.stat().st_size}, Content-Type: {content_type}"
            )
            response = httpx.post(
                url, headers=headers, files=files, data=upload_params, timeout=120.0
            )

        logger.info(
            f"Ответ Gokapi: статус={response.status_code}, заголовки={response.headers}, тело={response.text}"
        )
        ...
        if response.status_code == 200:
            try:
                ...
                if (
                    data.get("Result") == "OK"
                    and data.get("FileInfo")
                    and "UrlDownload" in data["FileInfo"]
                ):
                    ...
                    logger.warning(
                        f"Структура ответа Gokapi отличается, но UrlDownload найден: {data.get('UrlDownload')}"
                    )
                    ...
                    return (
                        False,
                        f"Ошибка Gokapi (неверный формат ответа): {data.get('ErrorMessage', 'Неизвестная ошибка')}",
                    )
            except ValueError:
            ...
            logger.error(
                f"Ошибка API: HTTP {response.status_code}, тело: {response.text}"
            )
            ...
            if response.status_code == 502:
                ...
                return (
                    False,
                    "Сервер загрузки перегружен. Попробуйте через несколько минут.",
                )
        ...
        logger.error(
            f"Неожиданная ошибка при загрузке файла на Gokapi: {str(e)}", exc_info=True
        )

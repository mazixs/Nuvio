...
- Повторный запуск `scripts/update_ytdlp.py` с уже закрепленной версией завершился без изменений файлов. Текущая версия из PyPI совпала с pin. Покрытие ветвей и строк в общем отчете - 62% при пороге CI 40%.

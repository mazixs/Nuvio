...
- `pytest tests/ -q`: 655 проверок прошли; `ruff check .`, `actionlint` и `git diff --check` прошли.
...
- Из свежего образа контрольное YouTube-видео скачано полностью: 37,0 МБ, временный файл удален. Публичный ролик с русским звуком также был скачан полностью: 38 299 221 байт.
...
- Отправка исходной записи VK размером 1,4 ГБ через локальный Telegram Bot API и повторная выдача из кэша. Полный файл здесь не скачивали и в Telegram не отправляли.

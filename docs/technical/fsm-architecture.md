...
### Отмена — состояние, а не украшение

Сессия заводится **до** разбора ссылки: без неё кнопке отмены не за что
зацепиться, и передумавшему оставалось бы только ждать. Отмена помечает
`session_id` в `utils/cancellation.py`, этот признак читает progress hook
yt-dlp — загрузка обрывается на месте, проверено на живом скачивании (301 МБ,
обрыв на 23% ровно в момент нажатия). После разбора ссылки `_finish_processing`
проверяет, что сессия ещё жива: если пользователь отменил, пока шёл разбор,
меню не показывается.

### Субтитры — тоже каскад

`main|subtitles` → язык (`format|subs_lang|<код>`) → формат
(`format|subs|<код>:<формат>`). С каждого экрана есть «Назад», причём с
формата — к языкам, а не в главное меню.

### Экран форматов теперь двухуровневый

`[FORMAT_MENU]` на схеме — это два экрана, а не один: сначала разделы
(**Видео / Аудио / Субтитры**, `main|video_menu`, `main|audio_menu`,
`main|subtitles`), затем сам выбор. Прежний плоский список выдавал не более
трёх combined, трёх «без звука» и двух аудио и дедуплицировал кнопки по тексту,
из-за чего из 22 форматов было видно шесть, а два формата одного разрешения
скрывали друг друга. Теперь на разрешение приходится одна кнопка, а из
`format`-действий остались только `combined` и `audio_only`.
...
**Состояние:** разбор callback-событий и хранилище сессий вынесены в
`utils/callback_fsm.py`; платформенные ключи, доставка и публичные ошибки также
имеют отдельные чистые модули. Координационные обработчики платформ всё ещё
остаются крупными и требуют дальнейшего дробления при добавлении новых сценариев.
...
- Сжатие файла сверх текущего лимита доставки тоже выполняется внутри
  скачивания (2 ГБ в Docker, 50 МБ при прямом облачном запуске)

**Чего нет:**
- Отложенной конвертации (скачать сырые фрагменты, конвертировать позже)
- Периодической очистки осиротевших файлов во время долгой работы
...
| 8 | **Типизированные callback-события** | Выполнено частично | 5 | 7 | 6 | **6.00** | `CallbackEvent` и `SessionStore` вынесены в `utils/callback_fsm.py`; таблицу обработчиков можно добавить при следующем расширении FSM. |

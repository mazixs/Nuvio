...
1. Отправьте ссылку на видео с поддерживаемой платформы.
2. Выберите доступный формат.
3. Дождитесь загрузки и отправки файла в чат.
...
⚠️ *Ограничения:*
- Обычно максимальная длительность видео: 3 часа. Длинные записи VK доступны,
  если их файл помещается в лимит отправки.
- Максимальный размер файла: 50 МБ через облачный Bot API или 2000 МБ через
  локальный Bot API. Бот использует режим, заданный администратором.
По вопросам и предложениям обращайтесь к администратору бота.
"""

SPAM_WARNING = (
    "⚠️ Слишком много запросов подряд. Повторите попытку через 10 секунд."
)
...
INVALID_URL_MESSAGE = (
    "❌ Не удалось распознать ссылку. Отправьте ссылку на видео с YouTube, "
    "TikTok, Instagram, Rutube или VK Video."
)
VK_NO_SUITABLE_FORMAT_MESSAGE = (
    "❌ Запись VK доступна, но размер подходящего прямого файла не удалось подтвердить. "
    "Лимит этого бота: {max_mb} МБ."
)
VK_LIVE_ACTIVE_MESSAGE = (
    "⏳ Трансляция VK еще идет. Отправьте ссылку после завершения эфира."
)
...
ERROR_FILE_TOO_LARGE_TELEGRAM = (
    "📁 Файл превышает лимит отправки этого бота в Telegram."
)
